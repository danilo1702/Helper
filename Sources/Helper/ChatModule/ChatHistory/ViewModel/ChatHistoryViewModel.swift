...
public class ChatHistoryViewModel: ObservableObject {
    ...
    func gettingChatHistory() {
        ...
        reference.addSnapshotListener { [weak self] querySnapshot, error in
            ...
            querySnapshot.documentChanges.forEach {  change in
                ...
                let message = self.convertToCardModel(message: messageModel, userUUID: uuid)
    ...
    func updateToReadChat(toUUID: String) {
        ...
        reference.getDocument { documentSnaphshot, error in
            ...
            if let status = snapshot.data()?[FirebaseConstants.messageRead] as? Bool {
                if status {
                    reference.updateData([FirebaseConstants.messageRead : false])
                }
            }
        }
    }
    
    func convertToCardModel(message: MessageModel, userUUID: String) -> CardModel  {
